    ...
    case 'ADD_USER_MSG': {
      ...
      return {
        ...
        sessions: {
          ...
          [action.key]: {
            ...session,
            courseId: action.courseId !== undefined ? action.courseId : session.courseId,
            messages: [
              ...session.messages,
              {
                id: nextOptimisticId(),
      ...
      return {
        ...
        sessions: {
          ...
          [action.key]: {
            ...
            courseId: action.courseId !== undefined ? action.courseId : existing.courseId,
            updatedAt: Date.now(),
          },
        },
      }
...
function asLLMSelection(value: unknown): LLMSelection | null {
  ...
  return profileId && modelId ? { profile_id: profileId, model_id: modelId } : null
}
...
function hydrateRequestSnapshot(
  ...
  const questionNotebookReferences = asQuestionReferences(stored.questionNotebookReferences)
  const persona =
    typeof stored.persona === 'string' && stored.persona.length > 0 ? stored.persona : ''
  ...
  const ensureRunner = useCallback(
    (key: string) => {
      ...
      if (existing) {
        ...
        if (session) {
          existing.client.setResumeState(session.activeTurnId, session.lastSeq, session.courseId)
        }
      ...
      if (session?.activeTurnId) {
        record.client.setResumeState(session.activeTurnId, session.lastSeq, session.courseId)
      }
