...
export default function FlashcardsWorkspace() {
  ...
  const activeDecks = useMemo(
    () => {
      const failedDeckIds = new Set(
        generationOperations
          .filter((operation) => operation.state === "failed")
          .map((operation) => operation.deck_id),
      );
      return decks.filter(
        (deck) =>
          deck.state !== "archived" &&
          !(deck.mode === "generated" && failedDeckIds.has(deck.id)),
      );
    },
    [decks, generationOperations],
  );
              ...
              <section className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
                ...
                groundedCreateStage === "confirming" ? (
                  <>
                    ...
                    {courseShell && !generationAvailable ? (
