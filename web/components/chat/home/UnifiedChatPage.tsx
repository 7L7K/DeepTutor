...
export interface UnifiedChatPageProps {
...
export default function UnifiedChatPage({
  ...
  const sessionCourseView = resolveSessionCourseView(courses, state.courseId, coursesLoading)
  ...
  const [learnerActionBusy, setLearnerActionBusy] = useState<CourseLearnerAction | null>(null)
  const [learnerActionError, setLearnerActionError] = useState<string | null>(null)
  const [practiceGenerationEnabled, setPracticeGenerationEnabled] = useState(false)
  const [flashcardGenerationEnabled, setFlashcardGenerationEnabled] = useState(false)
  ...
  useEffect(() => {
    if (state.sessionId && !sessionIdParam) {
      router.replace(
        courseRouteBase
          ? `${courseRouteBase}/${encodeURIComponent(state.sessionId)}`
          : `/home/${state.sessionId}`,
        { scroll: false }
      )
    }
  }, [state.sessionId, sessionIdParam, router, courseRouteBase])
  ...
  const handleLearnerAction = useCallback(
    async (action: CourseLearnerAction, assistantMessageId: number, desiredCount?: number) => {
      if (
        learnerActionBusy ||
        !courseIdentity ||
        !state.sessionId ||
      ...
      try {
        const plan = actionCourse
          ? await requestCourseLearnerAction(actionCourse.id, {
              ...
              sessionId: state.sessionId,
              assistantMessageId,
              idempotencyKey: ['learner-action', state.sessionId, assistantMessageId, action].join(
                ':'
              ),
        ...
        if (
          !isCurrentCourseLearnerAction(requested, learnerActionScopeRef.current) ||
          (requested.courseId != null && plan.course_id !== requested.courseId) ||
        ...
        if (isCurrentCourseLearnerAction(requested, learnerActionScopeRef.current)) {
          setLearnerActionError(
            cause instanceof Error ? cause.message : 'Could not start the Course learning action.'
          )
        }
      } finally {
        if (isCurrentCourseLearnerAction(requested, learnerActionScopeRef.current)) {
  ...
  useEffect(() => {
    ...
    const assistantIndex = [...state.messages]
      ...
      .reverse()
      .find(({ message }) => message.role === 'assistant' && message.id != null)?.index
    ...
    void handleLearnerAction('make_flashcards', Number(assistant.id), desiredCount)
  }, [courseMode, handleLearnerAction, learnerActionBusy, state.isStreaming, state.messages])
                  ...
                  >
                    <ChatMessageList
                      ...
                      generalFlashcardsEnabled={
                        !courseMode && Boolean(courseIdentity) && flashcardGenerationEnabled
                      }
